...
- (id)init {
	if( self = [super init]) {
		self.title=@"Messages";
		[self.tabBarItem initWithTitle:self.title image:[UIImage imageNamed:@"inbox.png"] tag:NULL];
...
- (void)viewDidAppear:(BOOL)animated {
	...
	cellSize = CGSizeMake([MessagesTable bounds].size.width, 60);	
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	[cell setText:[[Messages objectAtIndex: storyIndex] objectForKey: @"title"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSString * storyLink = [[Messages objectAtIndex: storyIndex] objectForKey: @"link"];
	NSString * storyTitle = [[Messages objectAtIndex: storyIndex] objectForKey: @"title"];
...
- (void)parserDidEndDocument:(NSXMLParser *)parser {
	...
	//NSLog(@"all done!");
	//NSLog(@"stories array has %d items", [Messages count]);
